...
import static com.map.app.service.TrafficAndRoutingService.ptRouter;
...
public class RoutePathContainer {
	...
	public ArrayList<RoutePath> findPublicTransportRoute(UrlContainer urlContainer, String profile) {
		ArrayList<RoutePath> result = new ArrayList<>();
		Request ghRequest = new Request(
				urlContainer.getStartlat(), urlContainer.getStartlon(), urlContainer.getEndlat(), urlContainer.getEndlon()
		);
		ghRequest.setEarliestDepartureTime(LocalDateTime.now().atZone(ZoneId.of("Asia/Kolkata")).toInstant());
		...
		ResponsePath res = route.getBest();
	...
	public ArrayList<RoutePath> find(UrlContainer p) {
		...
		switch (p.getVehicle()) {
			...
			case "metro":
				...
				break;
			case "pt":
				return findPublicTransportRoute(p,"pt");
			default:
